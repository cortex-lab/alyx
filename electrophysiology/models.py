...
class ExtracellularRecording(BaseExperimentalData):
    ...
    raw_data = models.ForeignKey(Dataset, blank=True, null=True, related_name="extracellular_recording_raw",
                                 help_text="Raw electrophysiology recording data in flat binary format")
    lowpass_data = models.ForeignKey(Dataset, blank=True, null=True, related_name="extracellular_recording_lpf",
                          help_text="Extracellular low-passed data")
    highpass_data = models.ForeignKey(Dataset, blank=True, null=True, related_name="extracellular_recording_hpf",
...
class SpikeSorting(BaseExperimentalData):
    ...
    mean_unfiltered_waveform = models.ForeignKey(Dataset, blank=True, null=True, related_name="spike_sorting_unfiltered_waveforms",
                                    help_text="mean unfiltered waveforms of every spike on every channel")
    mean_filtered_waveform = models.ForeignKey(Dataset, blank=True, null=True, related_name="spike_sorting_filtered_waveforms",
                                    help_text="mean filtered waveforms of every spike on every channel")
    generating_software = models.CharField(max_length=255, null=True, blank=True,
                                           help_text="e.g. “phy 0.8.3”")
    provenance_directory = models.ForeignKey(Dataset, blank=True, null=True, related_name="spike_sorting_provenance",
...
class IntracellularRecording(BaseExperimentalData):
    ...
    gain = models.FloatField(null=True, blank=True,
                             help_text="(A/V) – for info only; not required to convert raw data to amps")
    ...
    bridge_balance = models.FloatField(null=True, blank=True,
                                 help_text="(M Ohm)")
    recorded_voltage = models.ForeignKey(Dataset, blank=True, null=True, related_name="intracellular_recording_recorded_voltage",
                                    help_text="mV")
    current_command = models.ForeignKey(Dataset, blank=True, null=True, related_name="intracellular_recording_current_command",
                                    help_text="nA")
    gain = models.FloatField(null=True, blank=True,
                             help_text="(V/V) – for info only; not required to convert raw data to volts")