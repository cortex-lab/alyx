...
class SubjectListSerializer(serializers.HyperlinkedModelSerializer):
    ...
    class Meta:
        ...
        fields = ('nickname', 'url', 'responsible_user', 'birth_date', 'death_date',
                  'species', 'cage', 'sex', 'litter', 'strain', 'source', 'notes')
        lookup_field = 'nickname'
        extra_kwargs = {
            'url': {'view_name': 'subject-detail', 'lookup_field': 'nickname'}}


class SubjectDetailSerializer(SubjectListSerializer):

    weighings = WeighingListSerializer(many=True, read_only=True)
    water_administrations = WaterAdministrationListSerializer(
    ...
    class Meta:
        ...
        fields = ('nickname', 'url', 'responsible_user', 'birth_date', 'death_date',
                  'species', 'cage', 'sex',
                  'litter', 'strain', 'source', 'notes', 'actions_experiments',
                  'weighings', 'water_administrations')
        lookup_field = 'nickname'
        extra_kwargs = {
            'url': {'view_name': 'subject-detail', 'lookup_field': 'nickname'}}
