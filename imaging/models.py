...
class WidefieldImaging(BaseExperimentalData):
    ...
    description = models.CharField(max_length=255, null=True, blank=True,
                                       help_text="e.g. “field of view includes V1, S1, retrosplenial”")
...
class TwoPhotonImaging(BaseExperimentalData):
    raw_data = models.ForeignKey(Dataset, blank=True, null=True, related_name="two_photon_raw",
                                   help_text="array of size nT by nX by nY by nZ by nC")
    compressed_data = models.ForeignKey(Dataset, blank=True, null=True, related_name="two_photon_compressed",
    ...
    end_time = models.FloatField(null=True, blank=True,
                                 help_text="Equals start time if single application. TODO: should this be an offset? Or DateTimeField? Or TimeDifference?")
    imaging_indicator = models.CharField(max_length=255, null=True, blank=True,
                                         help_text="<GCaMP6f, GCaMP6m, GCaMP6s …>. TODO: normalize!")
    description = models.CharField(max_length=255, null=True, blank=True,
                                       help_text="e.g. “V1 layers 2-4”")
    image_position = models.ForeignKey(CoordinateTransformation, null=True, blank=True,
                                        help_text="Note if different planes have different alignment (e.g. flyback plane), this can’t be done in a single 3x3 transformation matrix, instead you would have an array of 3x2 matrices. TODO: how do we deal with this?")
...
class ROIDetection(BaseExperimentalData):
    ...
    plane = models.IntegerField(null=True, blank=True,
                                 help_text="array saying which plane each roi is found in. TODO: is this an ArrayField? JSON?")
    generating_software = models.CharField(max_length=255, null=True, blank=True,
                                       help_text="e.g. “AutoROI 0.8.3”")
    provenance_directory = models.ForeignKey(Dataset, blank=True, null=True, related_name="roi_detection_provenance",
                                   help_text="link to directory containing intermediate results")
    preprocessing = models.CharField(max_length=255, null=True, blank=True,
                                       help_text="computed (F-F0) / F0, estimating F0 as running min”")
...
class SVDCompressedMovie(BaseExperimentalData):
    compressed_data_U = models.ForeignKey(Dataset, blank=True, null=True, related_name="svd_movie_u",
                                   help_text="binary array containing SVD-compression eigenframes")
    compressed_data_V = models.ForeignKey(Dataset, blank=True, null=True, related_name="svd_movie_v",
                                   help_text="binary array containing SVD-compression timecourses")
