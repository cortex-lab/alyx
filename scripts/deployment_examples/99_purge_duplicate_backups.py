...
files_flatiron = {v.replace(str(FLAT_IRON_DIR) + '/', '').replace('alyxfull', 'alyx_full'): int(k)
                  for k, v in files_flatiron}

# Get all SQL backup files currently on the server
p = Popen(f"ls -s1 {Path(LOCAL_DIR).joinpath('2*/alyx_full.*.gz')}",
